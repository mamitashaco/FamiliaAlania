    ...
    <main className={`acceso ${oscuro ? "dark" : ""}`}>
      ...
      <section className="tarjeta-acceso">
        ...
        <form onSubmit={ingresar}>
          <label htmlFor="codigo">Código de acceso <b className="obligatorio">*</b></label>
          <input id="codigo" required inputMode="numeric" maxLength={8} placeholder="8 dígitos" value={codigo} onChange={(e) => setCodigo(e.target.value.replace(/\D/g, ""))} />
          <label htmlFor="clave">Contraseña <b className="obligatorio">*</b></label>
          <input id="clave" required type="password" placeholder="Tu contraseña" value={clave} onChange={(e) => setClave(e.target.value)} />
...
function VistaSalud() {
  ...
  return <>
    ...
    {formularioAbierto && <div className="velo"><section className="modal modal-ficha" role="dialog" aria-modal="true" aria-label={pestana === "perfil" ? "Editar perfil de salud" : "Nuevo registro de salud"}><div className="modal-cabecera"><div><div className="etiqueta">SALUD</div><h2>{pestana === "perfil" ? "Editar perfil de salud" : "Nuevo registro"}</h2></div><button className="boton-icono" onClick={() => setFormularioAbierto(false)} aria-label="Cerrar">×</button></div>
      {pestana === "perfil" ? <form className="formulario-salud" onSubmit={(e) => guardarSalud(e, "perfil")}><div className="campos"><label><span>Tipo de sangre</span><select name="tipo_sangre" defaultValue={perfil.tipo_sangre ?? ""}><option value="">Selecciona</option>{["O+","O-","A+","A-","B+","B-","AB+","AB-","No conoce"].map((x) => <option key={x}>{x}</option>)}</select></label>{[["seguro_medico", "Seguro médico"], ["alergias", "Alergias"], ["enfermedades_relevantes", "Enfermedades relevantes"], ["medicacion_habitual", "Medicación habitual"], ["medico_referencia", "Médico de referencia"]].map(([campo, etiqueta]) => <label key={campo}><span>{etiqueta}</span><input name={campo} defaultValue={perfil[campo] ?? ""} /></label>)}</div>{avisoSalud && <p className={avisoSalud.startsWith("✓") ? "exito-modal" : "error-modal"}>{avisoSalud}</p>}<div className="modal-acciones"><button type="button" className="secundario" onClick={() => setFormularioAbierto(false)}>Cerrar</button><button className="primario">Guardar</button></div></form> :
      <form className="formulario-salud" onSubmit={(e) => guardarSalud(e, pestana)}><div className="campos">{CAMPOS_SALUD[pestana].map(([campo, etiqueta, tipo]) => { const obligatorios: Record<string, string[]> = { historial: ["fecha", "diagnostico", "tratamiento"], medicamentos: ["nombre"], vacunas: ["nombre", "fecha_aplicacion"], examenes: ["nombre", "fecha"], signos: ["registrado_en"] }; const requerido = obligatorios[pestana]?.includes(campo) ?? false; return <label key={campo} className={tipo === "textarea" ? "ancho" : ""}><span>{etiqueta}{requerido && <> <b className="obligatorio">*</b></>}</span>{tipo === "textarea" ? <textarea name={campo} rows={4} required={requerido} /> : tipo === "unidad" ? <select name={campo} defaultValue="semanas" required={requerido}><option value="semanas">Semanas</option><option value="meses">Meses</option></select> : <input name={campo} type={tipo === "date" || tipo === "number" ? tipo : "text"} list={tipo === "sugerencia" ? "tipos-atencion" : undefined} step={tipo === "number" ? "0.01" : undefined} defaultValue={tipo === "date" && !["fecha_fin", "proxima_fecha", "proximo_control"].includes(campo) ? new Date().toISOString().slice(0, 10) : undefined} required={requerido} />}</label>; })}</div>{pestana === "historial" && <fieldset className="medicamentos-tratamiento"><legend>Medicamentos del tratamiento</legend><div className="tabla-medicamentos"><div className="tabla-medicamentos-cabecera"><span>Medicamento</span><span>Dosis</span><span>Frecuencia</span><span>Indicaciones</span><span /></div>{medicamentosTratamiento.map((m, i) => <div className="tabla-medicamentos-fila" key={i}><input value={m.nombre} placeholder="Nombre" onChange={(e) => setMedicamentosTratamiento(medicamentosTratamiento.map((x, n) => n === i ? { ...x, nombre: e.target.value } : x))} /><input value={m.dosis} placeholder="Dosis" onChange={(e) => setMedicamentosTratamiento(medicamentosTratamiento.map((x, n) => n === i ? { ...x, dosis: e.target.value } : x))} /><input value={m.frecuencia} placeholder="Cada 8 horas" onChange={(e) => setMedicamentosTratamiento(medicamentosTratamiento.map((x, n) => n === i ? { ...x, frecuencia: e.target.value } : x))} /><input value={m.indicaciones} placeholder="Indicaciones" onChange={(e) => setMedicamentosTratamiento(medicamentosTratamiento.map((x, n) => n === i ? { ...x, indicaciones: e.target.value } : x))} />{medicamentosTratamiento.length > 1 && <button type="button" className="quitar" onClick={() => setMedicamentosTratamiento(medicamentosTratamiento.filter((_, n) => n !== i))}>×</button>}</div>)}</div><button type="button" className="secundario agregar-medicamento" onClick={() => setMedicamentosTratamiento([...medicamentosTratamiento, { nombre: "", dosis: "", frecuencia: "", indicaciones: "" }])}>＋ Agregar medicamento</button></fieldset>}<datalist id="tipos-atencion">{Array.from(new Set(datosSalud.flatMap((p) => (p.tb_historial_medico ?? []).map((r: Record<string, string>) => r.tipo).filter(Boolean))).values()).map((x) => <option key={String(x)} value={String(x)} />)}</datalist>{avisoSalud && <p className={avisoSalud.startsWith("✓") ? "exito-modal" : "error-modal"}>{avisoSalud}</p>}<div className="modal-acciones"><button type="button" className="secundario" onClick={() => setFormularioAbierto(false)}>Cerrar</button><button className="primario">Guardar registro</button></div></form>}</section></div>}
  </>;
}
...
function VistaConfiguracion({ onChanged }: { onChanged: () => void }) {
  ...
  return <>
    ...
    <section className="tarjeta configuracion-lista">{usuarios.map((u, i) => <form key={u.id} onSubmit={(e) => { e.preventDefault(); guardarUsuario(u); }}>
      ...
      <label><span>Nombre completo <b className="obligatorio">*</b></span><input required value={u.nombre_completo} onChange={(e) => setUsuarios(usuarios.map((x, n) => n === i ? { ...x, nombre_completo: e.target.value } : x))} /></label>
      <label><span>{u.usuario_id ? "Código de acceso" : "Asignar código de acceso"}{u.usuario_id && <> <b className="obligatorio">*</b></>}</span><input required={Boolean(u.usuario_id)} inputMode="numeric" maxLength={8} value={u.codigo} placeholder="8 dígitos" onChange={(e) => setUsuarios(usuarios.map((x, n) => n === i ? { ...x, codigo: e.target.value.replace(/\D/g, "") } : x))} /></label>
      <label><span>Rol <b className="obligatorio">*</b></span><select required value={u.rol ?? "integrante"} onChange={(e) => setUsuarios(usuarios.map((x, n) => n === i ? { ...x, rol: e.target.value as "administrador" | "integrante" } : x))}><option value="integrante">Integrante</option><option value="administrador">Administrador</option></select></label>
...
function ModalNuevoIntegrante({ onClose, onSaved, esAdministrador }: { onClose: () => void; onSaved: () => void; esAdministrador: boolean }) {
  ...
  return <div className="velo" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="modal modal-corta" role="dialog" aria-modal="true" aria-label="Nuevo integrante">
    ...
    <form onSubmit={crear}><div className="campos">
      <label className="ancho"><span>Nombre completo <b className="obligatorio">*</b></span><input name="nombre_completo" required autoFocus placeholder="Nombres y apellidos" /></label>
      {!esAdministrador && <label className="ancho"><span>Parentesco contigo <b className="obligatorio">*</b></span><select name="parentesco" required defaultValue=""><option value="" disabled>Selecciona una relación</option>{RELACIONES.map((r) => <option key={r}>{r}</option>)}</select></label>}
...
function ModalFicha({ integrante, puedeEditar, onClose, onSaved }: { integrante: Integrante; puedeEditar: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="velo" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="modal modal-ficha" role="dialog" aria-modal="true" aria-label={`Ficha de ${integrante.nombre}`}>
    ...
    <form onSubmit={guardarFicha}>{seccionesFicha.map(([titulo, campos]) => <fieldset key={titulo}><legend>{titulo}</legend><div className="campos">{campos.map(([nombre, etiqueta, tipo]) =>
      <label key={nombre}><span>{etiqueta}{nombre === "nombre" && <> <b className="obligatorio">*</b></>}</span><input name={nombre} required={nombre === "nombre"} type={tipo ?? "text"} defaultValue={String(integrante[nombre as keyof Integrante] ?? "")} disabled={!puedeEditar} /></label>
    )}</div></fieldset>)}
...
function Modal({ titulo, seccion, onClose, onSave }: { titulo: string; seccion: string; onClose: () => void; onSave: (e: FormEvent<HTMLFormElement>) => void }) {
  return <div className="velo" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="modal" role="dialog" aria-modal="true" aria-label={titulo}><div className="modal-cabecera"><div><div className="etiqueta">NUEVO REGISTRO</div><h2>{titulo}</h2></div><button className="boton-icono" onClick={onClose} aria-label="Cerrar">×</button></div><form onSubmit={onSave}>
    <div className="campos">
      <label><span>{seccion === "Precios" ? "Descripción del producto" : "Título"} <b className="obligatorio">*</b></span><input required placeholder={seccion === "Precios" ? "Ej. Aceite vegetal" : "Nombre del registro"} /></label>
      <label><span>{seccion === "Precios" ? "Categoría" : "Fecha"} <b className="obligatorio">*</b></span><input required type={seccion === "Precios" ? "text" : "date"} /></label>
      {seccion === "Precios" && <><label><span>Precio (S/) <b className="obligatorio">*</b></span><input required type="number" step="0.01" placeholder="0.00" /></label><label><span>Presentación</span><input placeholder="Ej. Botella 1 L" /></label><label><span>Costo unitario</span><input type="number" step="0.01" placeholder="0.00" /></label><label><span>Tienda <b className="obligatorio">*</b></span><input required placeholder="Nombre de la tienda" /></label></>}
